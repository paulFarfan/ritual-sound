        ...
        <div className="p-5">
          ...
          {!isCatalog && !isFeatured && (
            <p className="text-xs text-white/50">
              {dj.profile.location?.[0]} • {dj.sound.genres}
            </p>
          )}
          ...
          {(isCatalog || isFeatured) && (
            <>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {dj.sound.genres?.slice(0, 2).map((s) => (
                  <span
                    key={s}
              ...
              {/* LOCATION */}
              <p className="text-xs text-white/40 mt-3">{dj.location?.[0]}</p>
            </>
          )}

          {/* INSTAGRAM (solo minimal) */}
          {!isCatalog && !isFeatured && (
            <a
